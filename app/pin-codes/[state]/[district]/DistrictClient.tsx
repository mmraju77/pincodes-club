...
const getDistrictDescription = (districtName: string, stateName: string) => {
  ...
  if (templateIndex === 0) {
    return <p>Welcome to the official postal directory for <strong>{dName}</strong> district in the state of <strong>{sName}</strong>. <strong>Pincode Club</strong> provides the most reliable and lightning-fast search platform to find exact <strong>{dName} PIN codes</strong>, post office locations, and banking IFSC details. Scroll down to seamlessly navigate through all the postal divisions and discover the exact location information you are searching for.</p>;
  } else if (templateIndex === 1) {
    return <p>Finding reliable postal information for <strong>{dName}</strong> in <strong>{sName}</strong> is now simpler than ever. At <strong>Pincode Club</strong>, our mission is to offer you a seamless experience when searching for <strong>{dName} pin codes</strong>. Explore the comprehensive data below to quickly locate the exact PIN or IFSC code you need anywhere within the {dName} region.</p>;
  } else {
    return <p>Are you looking for accurate postal data in <strong>{dName}</strong>, <strong>{sName}</strong>? You have come to the right place! <strong>Pincode Club</strong> is India's ultimate directory hub, meticulously organizing every single post office within the <strong>{dName}</strong> district. Scroll down to seamlessly navigate through all the postal divisions and discover the exact location information you are searching for.</p>;
  }
};
...
export default function DistrictClient() {
  ...
  const fetchPincodes = async (districtName: string) => {
    setIsLoading(true);
    try {
      // Speed Optimization: Querying database directly by exact district column match (Instant 1-second load time)
      const { data, error } = await supabase
        .from('pincodes')
        .select('pincode, officename, officetype, district, divisionname')
        .ilike('district', `%${districtName.trim()}%`);
        
      if (error) throw error;
      
      if (data) {
        const uniqueData = data.filter((v, i, a) => a.findIndex(t => (t.pincode === v.pincode && t.officename === v.officename)) === i);
    ...
    <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 min-h-screen space-y-8">
      <div className="bg-[#0f172a] p-6 md:p-8 rounded-2xl border border-slate-800 shadow-xl flex flex-col lg:flex-row justify-between items-center gap-6">
        <div className="flex-1 text-center lg:text-left">
          <div className="flex items-center gap-2 flex-wrap justify-center lg:justify-start mb-3">
            <Link href="/pin-codes" className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-lg transition-colors text-xs">ALL STATES</Link>
            <span className="text-slate-600 font-bold text-sm">&rarr;</span>
            <Link href={`/pin-codes/${rawState}`} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-lg transition-colors text-xs uppercase">{decodedState}</Link>
            <span className="text-slate-600 font-bold text-sm">&rarr;</span>
            <span className="px-3 py-1.5 bg-orange-500/20 text-orange-400 border border-orange-500/30 font-bold rounded-lg text-xs uppercase">{decodedDistrict}</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tight">PIN Codes in {decodedDistrict.toUpperCase()}</h1>
        ...
        <div className="w-full lg:w-[400px]">
          <div className="relative group">
            ...
            <div onClick={startListening} className={`absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer transition-colors ${isListening ? 'text-red-500 animate-pulse' : 'text-slate-500 hover:text-orange-400'}`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11a7 7 0 01-7-7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 11-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
            </div>
          </div>
        </div>
      </div>

      {seoContent && <div className="bg-slate-900/50 p-5 md:p-6 rounded-2xl border border-slate-800 shadow-sm text-slate-300 leading-relaxed text-sm">{seoContent}</div>}
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          {searchQuery && <h2 className="text-xl font-bold text-white border-b border-slate-800 pb-3">Post Offices matching "{searchQuery}"</h2>}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {currentResults.map((item, index) => (
              <Link key={index} href={`/pin-codes/${encodeURIComponent(decodedState)}/${encodeURIComponent(decodedDistrict)}/${item.pincode}`} className="group block h-full">
                <div className="bg-[#0f172a] p-4 rounded-xl border border-slate-800 hover:border-orange-500/50 transition-all cursor-pointer h-full shadow-md flex flex-col justify-between">
                  ...
                  <div className="mt-auto text-xs text-slate-400"><p className="line-clamp-1">Division: <span className="text-slate-200">{item.divisionname || 'N/A'}</span></p></div>
                </div>
              </Link>
            ))}
          </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-3 pt-6 border-t border-slate-800/50">
              <button onClick={() => { setCurrentPage(p => Math.max(1, p - 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === 1} className="px-4 py-2 bg-[#0f172a] border border-slate-800 hover:bg-slate-800 disabled:opacity-50 text-white font-bold rounded-lg transition-colors text-sm">&larr; Prev</button>
              <span className="text-slate-400 font-medium bg-[#0f172a] px-4 py-2 rounded-lg border border-slate-800 text-sm"><span className="text-white font-bold">{currentPage}</span> / {totalPages}</span>
              <button onClick={() => { setCurrentPage(p => Math.min(totalPages, p + 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={currentPage === totalPages} className="px-4 py-2 bg-[#0f172a] border border-slate-800 hover:bg-slate-800 disabled:opacity-50 text-white font-bold rounded-lg transition-colors text-sm">Next &rarr;</button>
            </div>
          )}
        </div>
      ) : (
