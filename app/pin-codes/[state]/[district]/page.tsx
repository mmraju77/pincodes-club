  ...
  useEffect(() => {
    async function fetchOffices() {
      const { data } = await supabase
        ...
        // 🛠️ MAGIC SORTING: S.O and H.O will appear before B.O alphabetically if we sort descending
        .order('officetype', { ascending: false });
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-4 sm:px-6 min-h-screen">
      ...
      <div className="bg-gradient-to-r from-purple-900/40 to-slate-900 border border-slate-700 rounded-3xl p-8 md:p-10 mb-10 shadow-2xl">
        ...
        <p className="text-slate-400">Showing {offices.length} registered Post Offices (Sorted by Main Offices first)</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {offices.map((item, index) => (
             // 🛠️ ADDED LINK: Now cards are clickable and go to the specific Pincode Page
             <Link 
               href={`/pin-codes/${encodeURIComponent(stateName)}/${encodeURIComponent(districtName)}/${item.pincode}`}
               key={index} 
             ...
             <div className="grid grid-cols-2 gap-4">
               <div>
                 ...
                 <span className={`text-sm font-bold ${item.officetype === 'S.O' || item.officetype === 'H.O' ? 'text-purple-400' : 'text-slate-300'}`}>
                   {item.officetype || 'N/A'}
                 </span>
               </div>
               <div>
