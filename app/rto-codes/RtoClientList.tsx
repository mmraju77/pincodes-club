...
export default function RtoClientList({ rtoData }: { rtoData: any[] }) {
  ...
  // 🔍 BULLETPROOF SEARCH: Searches Code, Place, and State!
  ...
  return (
    <div className="w-full space-y-8">
      ...
      <div className="relative max-w-2xl mx-auto mb-10">
        ...
        <input
          type="text"
          placeholder="Search by RTO Code (e.g., TS09), City, or State..."
          value={searchTerm}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filteredData.length > 0 ? (
          ...
          <div className="col-span-full py-16 text-center bg-slate-900/30 rounded-2xl border border-slate-800 border-dashed">
            <p className="text-slate-400 text-lg">No RTO codes found matching "{searchTerm}"</p>
          </div>
        )}
      </div>
    </div>
