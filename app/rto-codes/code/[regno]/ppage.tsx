...
export default async function RtoDetailsPage({ params }: Props) {
  ...
  // 🚀 BULLETPROOF FETCH: Avoids Supabase crashes due to duplicates or trailing spaces
  ...
  if (error || !rtoData) {
    // 🛡️ Custom Error Page instead of Generic Post Office Not Found
    return (
      <div className="max-w-4xl mx-auto py-24 px-4 text-center min-h-screen">
        <h1 className="text-4xl text-red-400 font-extrabold mb-4">Code Not Found</h1>
        <p className="text-slate-400 text-lg">Could not locate data for RTO Code: {decodedRegno}</p>
        <Link href="/rto-codes" className="mt-8 inline-block bg-purple-600 hover:bg-purple-500 px-8 py-3 rounded-full text-white font-bold transition-all shadow-lg shadow-purple-500/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
            <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700/50">
              ...
              <Link href={`/rto-codes/${stateSlug}`} className="text-xl font-bold text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-2">
                {rtoData.state}
                <span className="text-xs bg-blue-500/20 px-2 py-1 rounded ml-2">View All ➔</span>
              </Link>
            </div>
